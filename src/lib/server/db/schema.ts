...
export const careers = mysqlTable('careers', {
	...
	clave: varchar('clave', { length: 5 })
		.notNull()
		.generatedAlwaysAs((): SQL => sql`concat(${careers.nucleo},${careers.ofae})`, {
			mode: 'virtual'
		})
});
...
export const APActiveDates = mysqlTable(
	'ap_active_dates',
	{
...
export const auditEvents = mysqlTable(
	'audit_events',
	{
		sequence: bigint('sequence', { mode: 'number', unsigned: true }).primaryKey(),
	...
	(table) => [
		unique('audit_events_event_id_unique').on(table.eventId),
		index('audit_events_occurred_idx').on(table.occurredAt),
		index('audit_events_action_idx').on(table.action, table.outcome),
...
export const auditChainState = mysqlTable('audit_chain_state', {
	id: int('id').primaryKey(),
	lastSequence: bigint('last_sequence', { mode: 'number', unsigned: true }).notNull(),
...
export const admissionImportBatches = mysqlTable(
...
export const admissions = mysqlTable(
	...
	{
		id: serial('id').primaryKey(),
		importBatchId: bigint('import_batch_id', { mode: 'number', unsigned: true }).references(
			() => admissionImportBatches.id
		),
		...
		// To which career
		careerId: bigint('career_id', { mode: 'number', unsigned: true })
			.notNull()
			.references(() => careers.id),

		// Under which admission process (num_asignacion → last active code)
		procesoId: bigint('proceso_id', { mode: 'number', unsigned: true })
			.notNull()
			.references(() => proceso_admission.id),

		// Via which modality  (Moda_Ingre: PS / OPS / CP → mod_admission.code)
		modalityId: bigint('modality_id', { mode: 'number', unsigned: true })
			.notNull()
			.references(() => mod_admission.id),
		...
		ocreTypeId: bigint('ocre_type_id', { mode: 'number', unsigned: true }).references(
			() => ocre_types.id
		),
	...
	(table) => ({
		...
		fechaAsignacionIdx: index('admissions_fecha_asignacion_idx').on(table.fechaAsignacion),
		periodoFechaIdx: index('admissions_periodo_fecha_idx').on(
			table.periodoIngreso,
			table.fechaAsignacion
		)
	})
);

export const admissionProcessings = mysqlTable(
	'admission_processings',
	{
		id: serial('id').primaryKey(),
		admissionId: bigint('admission_id', { mode: 'number', unsigned: true })
			.notNull()
			.unique()
			.references(() => admissions.id),
		processingDate: date('processing_date').notNull(),
		planillaGeneratedAt: timestamp('planilla_generated_at', { fsp: 3 }).defaultNow().notNull(),
		...
		reviewedByUserId: varchar('reviewed_by_user_id', { length: 36 }).references(() => user.id),
		returnReason: varchar('return_reason', { length: 500 }),
		unlockedAt: timestamp('unlocked_at', { fsp: 3 }),
		unlockedByUserId: varchar('unlocked_by_user_id', { length: 36 }).references(() => user.id),
		unlockReason: varchar('unlock_reason', { length: 500 })
	},
	(table) => [
		index('admission_processings_date_status_idx').on(table.processingDate, table.status),
		index('admission_processings_generated_by_idx').on(table.generatedByUserId)
	]
);
