import { buildStatisticsPdf, makeFolio } from '$lib/server/admisiones/statistics-export';
import {
...
export const GET: RequestHandler = async (event) => {
	...
	const generatedBy = event.locals.user.name ?? event.locals.user.email ?? undefined;
	...
	const pdf = await buildStatisticsPdf(report, { folio, generatedBy });
	const body = pdf.buffer.slice(pdf.byteOffset, pdf.byteOffset + pdf.byteLength) as ArrayBuffer;
	return new Response(body, {
		headers: {
			'content-type': 'application/pdf',
			'content-disposition': `attachment; filename="${exportFileStem(report)}.pdf"`,
